...
ammoniteは、ScalaのモダンなREPLや`sbt`を使うよりも簡単なセットアップで
Scalaスクリプトを書けるScript、ファイルシステムやshellのことだ.

例えば、以下のような`example.sc`ファイルを用意するだけでScalaスクリプトを実行できる.
...
このammoniteを使うとScalaの開発体験が爆上がりするので紹介する.
