...
export type Unnamed_3_Query = { site?: Maybe<{ siteMetadata?: Maybe<(
      Pick<SiteSiteMetadata, 'siteTitle' | 'siteTitleAlt' | 'siteDescription' | 'siteUrl' | 'siteLanguage' | 'author' | 'favicon'>
      & { social?: Maybe<Pick<SiteSiteMetadataSocial, 'twitter' | 'github'>> }
    )> }> };
...
export type GatsbyNodeQuery = { allMdx: { edges: Array<{ node: (
        Pick<Mdx, 'id' | 'tableOfContents' | 'body' | 'excerpt'>
        & { frontmatter?: Maybe<Pick<MdxFrontmatter, 'path' | 'status' | 'tags' | 'title' | 'date' | 'description'>> }
      ), next?: Maybe<{ frontmatter?: Maybe<Pick<MdxFrontmatter, 'path' | 'title'>> }>, previous?: Maybe<{ frontmatter?: Maybe<Pick<MdxFrontmatter, 'path' | 'title'>> }> }> } };
